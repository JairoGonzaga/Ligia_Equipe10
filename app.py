...
def check_environment():
    ...
    try:
        ...
        if sklearn.__version__ != '1.6.1':
            ...
            print("   O modelo foi treinado com versão 1.6.1")
            print("   Recomendado usar ambiente virtual com: scikit-learn==1.6.1")
            print("   Execute: .\\venv\\Scripts\\Activate.ps1")
            return False
        return True
    except ImportError:
        print("scikit-learn não encontrado!")
        print("   Execute: pip install scikit-learn==1.6.1")
        return False
...
def load_model_1_6_1():
    """Carrega o modelo compatível com scikit-learn 1.6.1"""
    try:
        model_path = os.path.join("Model", "Modelo_Inferencia.joblib")
        
        # Verificar se o arquivo existe
        if not os.path.exists(model_path):
            # Tentar caminhos alternativos
            alternative_paths = [
                "Modelo_Inferencia.joblib",
                "../Model/Modelo_Inferencia.joblib",
                os.path.join(os.path.dirname(__file__), "Model", "Modelo_Inferencia.joblib")
            ]
        ...
        return model, "✅ Modelo original carregado"
        
    except Exception as e:
        st.error(f"Erro ao carregar modelo original: {str(e)}")
...
if model is not None:
    ...
    **Solução recomendada:**
    ...
    1. **Verifique se o arquivo do modelo existe:**
       - Caminho: `Model/Modelo_Inferencia.joblib`
